...
def register_week_events(
    *,
    ...
    """Discover the provider's events in a window and register them.

    The provider comes from the season's active `SeasonRules`, never from
    an operator flag: a registration run that used a different feed from
    the one the season is pinned to would create Game rows no capture could
    ever refresh.
    """
...
def render(report: RegistrationReport) -> str:
    ...
    add = out.append
    add("=" * 78)
    add("WEEK EVENT REGISTRATION — identity only, no quotes, no roster")
    add("=" * 78)
    add(f"  season            {report.season_name}  ({report.season_id})")
    add(f"  week              {report.week_number}")
    add(f"  provider          {report.provider}  (from active SeasonRules)")
    add(f"  window            {report.window_start}  ..  {report.window_end}")
    add("")
    ...
    if report.all_games:
        ...
        for g in report.all_games:
            flag = "NEW " if g.created else "kept"
            add(f"  {flag} {g.away} @ {g.home}  {g.kickoff_at.isoformat()}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--days-ahead", type=int, default=8,
        help="how far forward to look for events. The window is now .. now+N days.",
    )
