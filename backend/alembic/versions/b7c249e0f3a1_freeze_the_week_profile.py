...
That is not hypothetical. The first attempt to deploy this revision hit
exactly that abort: production held four pre-profile smoke weeks in the
`is_real_money=False` / standings=True / awards=True shape. The guard was
right and the release process was wrong -- the discovery happened at
container start, after Railway had already removed the healthy deployment,
so the abort became an outage. Those four rows were then diagnosed and
repaired explicitly by c5d83b1e7a42, which is why this revision now sits
on top of it. The preflight stays exactly as it was; a schema preflight
against production data now runs BEFORE the deployment is replaced.
"""
...
down_revision: Union[str, Sequence[str], None] = "c5d83b1e7a42"
